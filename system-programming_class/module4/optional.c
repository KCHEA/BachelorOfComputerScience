...
main(int ac, char *av[])
{
    ...
    //we will as user if they want to overwrite the original file
    printf("You will overwrite file: %s\n",av[1]);
    printf("Y or N: ");
    char answer;
    scanf(" %c", &answer);
    while(answer == '\0' || (answer != 'Y' && answer != 'y') && (answer != 'n' && answer != 'N')){
        printf("Invalid input, please enter either Y or N: ");
        scanf(" %c", &answer);
    }
    if(answer == 'n' || answer == 'N'){
        printf("File will not be copy\n");
        exit(1);
    }
